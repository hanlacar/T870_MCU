...
// ★ v38 변경 — 가속은 램프 없이 즉시, 감속만 램프.
//
//   왜 가속 램프를 없앴나
//     뒷바퀴 구동계에는 엔코더가 없어 PWM 을 그대로 인가한다.
//     앞바퀴만 램프를 타면 출발 순간 앞뒤가 서로 다른 힘으로 밀어
//     오픈 디퍼렌셜에서 한쪽이 헛돈다. 앞뒤를 같은 순간 같은 값으로
//     인가하는 편이 오히려 슬립이 적고, 측정용 스케치(CAL)와도 동작이 같아져
//     거기서 잰 counts_per_meter 를 그대로 쓸 수 있다.
//
//   왜 감속 램프는 남겼나
//     PWM 을 0 으로 뚝 떨어뜨리면 Cytron 이 하단을 단락해 전자 브레이크가
//     걸린다. 그 순간 역토크가 커서 차가 앞으로 쏠리고 타이어가 끌린다.
//     감속은 부드럽게 빼는 편이 정지 거리도 일정하다.
//     급정거가 필요할 때는 램프가 아니라 B(제동 펄스) 명령을 쓴다.
constexpr uint8_t       DRIVE_ACCEL_STEP        = 0;   // 0 = 램프 없음 (즉시 인가)
...
void updateDriveController(unsigned long now)
{
  ...
  // ★ v38 — 가속은 50ms 게이트보다 먼저 처리한다.
  //   게이트 안쪽에 두면 명령을 받고도 최대 50ms 를 기다렸다 나간다.
  //   "바로바로" 가 목적이므로 그 지연도 없앤다.
  if (DRIVE_ACCEL_STEP == 0 && currentDrivePwm < targetDrivePwm) {
    applyDrivePwm(targetDrivePwm);
    lastDriveRampMs = now;
    return;
  }
