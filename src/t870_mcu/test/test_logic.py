...
# ============================================================
# 0831 — 회피 게이트 조향 권한 이양
# ============================================================

def test_gate_owner_takes_wheel_when_open():
    """게이트가 열리면 게이트 소스(라이다)가 조향을 가져간다."""
    inputs = InputManager(["lidar", "camera", "gps"])
    gate = WheelGate("camera", [], ["lidar", "camera", "gps"],
                     ["0", "5", "7"])
    now = 100.0
    inputs.update_wheel("camera", 5, now, True, "ok")
    inputs.update_wheel("lidar", -20, now, True, "ok")
    ...
    # 게이트 닫힘 → 기본 소유자(카메라)
    value, used, ok, _ = gate.resolve("5", inputs, now, 1.0, 0, None)
    assert (value, used, ok) == (5, "camera", True)

    # 게이트 열림 → 라이다로 이양
    value, used, ok, _ = gate.resolve("5", inputs, now, 1.0, 0, "lidar")
    assert (value, used, ok) == (-20, "lidar", True)


def test_gate_owner_none_keeps_static_table():
    """gate_owner 가 없으면 예전처럼 모드별 정적 표를 그대로 쓴다."""
    inputs = InputManager(["lidar", "camera"])
    gate = WheelGate("camera", ["7:lidar"], ["lidar", "camera"],
                     ["0", "5", "7"])
    now = 100.0
    inputs.update_wheel("camera", 3, now, True, "ok")
    inputs.update_wheel("lidar", 11, now, True, "ok")

    assert gate.resolve("5", inputs, now, 1.0, 0)[1] == "camera"
    assert gate.resolve("7", inputs, now, 1.0, 0)[1] == "lidar"


def test_gate_owner_overrides_static_table():
    """게이트 이양은 정적 override 보다 우선한다."""
    inputs = InputManager(["lidar", "camera"])
    gate = WheelGate("camera", ["5:camera"], ["lidar", "camera"],
                     ["0", "5"])
    now = 100.0
    inputs.update_wheel("lidar", 9, now, True, "ok")
    assert gate.resolve("5", inputs, now, 1.0, 0, "lidar")[1] == "lidar"


def test_gate_owner_failsafe_when_source_silent():
    """이양받은 소스가 값을 안 주면 폴백하고 '정상 아님'을 알린다."""
    inputs = InputManager(["lidar", "camera"])
    gate = WheelGate("camera", [], ["lidar", "camera"], ["0", "5"])
    now = 100.0
    value, used, ok, reason = gate.resolve("5", inputs, now, 1.0, 0, "lidar")
    assert value == 0 and used == FAILSAFE and ok is False
    assert "lidar" in reason


def test_new_mode_table_is_0_to_11():
    """0831 구간표: 0~11 전부 알려진 모드여야 한다."""
    gate = WheelGate("camera", ["7:lidar", "10:lidar"],
                     ["lidar", "camera", "gps"],
                     [str(i) for i in range(12)])
    for i in range(12):
        assert gate.is_known(str(i))
    assert not gate.is_known("12")
    assert gate.owner("7") == "lidar"
