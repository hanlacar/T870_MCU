#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""run_v4_0830.py — 정한 시간만큼 전진하고 멈춘다. 조향은 절대 건드리지 않는다.

    python3 tools/run_v4_0830.py            2단 5초
    python3 tools/run_v4_0830.py 15         2단 15초
    python3 tools/run_v4_0830.py 15 3       3단 15초
...
STOP = "1.00"
STAGE = {1: "2.00", 2: "3.00", 3: "4.00"}

#  인자:  [초] [단]      기본 5초 2단
RUN_SEC = float(sys.argv[1]) if len(sys.argv) > 1 else 5.0
_stage = int(sys.argv[2]) if len(sys.argv) > 2 else 2
DRIVE = STAGE.get(_stage)
if DRIVE is None or not (0 < RUN_SEC <= 40):
    sys.exit("사용법: run_v4_0830.py [초 0~40] [단 1~3]")
COAST_SEC = 3.0
SEND_HZ = 10.0
...
print("\n%d단으로 %.0f초 전진한다. 조향은 건드리지 않는다." % (_stage, RUN_SEC))
...
total = run + coast
print("  총(주행+코스팅) %d 카운트  ← 줄자는 멈춘 자리까지 재므로 이쪽과 짝" % total)
print("  ★ 줄자 거리 D 라면  counts_per_meter = %d / D" % total)
print("  ")
print("  두 모델의 예측 (D 를 넣어 비교할 것)")
print("    고정분+비례  총카운트 ≈ 1934 + 315 x D")
print("    단순비       총카운트 ≈ 825 x D")
print("  --------------------------------------")
