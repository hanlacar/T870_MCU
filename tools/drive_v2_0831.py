#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""drive_v2_0831.py — 혼자 쓰는 수동 조종기. ROS 불필요, 시리얼 직결.

    python3 tools/drive_v2_0831.py
...
키
  ── 주행 ─────────────────────────────
    ...
    s  후진 1단     x  후진 2단
    [스페이스]  정지
    ※ 시간 제한 없다. 스페이스나 0(즉시정지)을 누를 때까지 계속 간다.
      단 이 프로그램이 죽으면 명령이 끊기고, 펌웨어 워치독이 2초 안에
      차를 세운다. 그게 마지막 안전망이다.
...
#  ★ 자동 차단 없음 (v2). 스페이스를 누를 때까지 계속 간다.
#    프로그램이 죽으면 명령이 끊기고 펌웨어 워치독(2초)이 세운다.
