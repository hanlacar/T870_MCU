#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""run5_v2_0830.py — 2단으로 딱 5초 전진하고 멈춘다. 조향은 절대 건드리지 않는다.

    python3 tools/run5_v2_0830.py
...
ser.timeout = 1
ser.dtr = False
ser.rts = False          # v2: RTS 도 함께 내린다 (보드에 따라 이쪽이 리셋선이다)
try:
    ser.open()
except Exception as exc:
    sys.exit("포트를 못 열었다: %s\n  다른 프로그램이 잡고 있는지 확인:\n"
             "    pgrep -af 'mcu_bridge|center|run10|serial_console'" % exc)
print("포트 %s  (DTR/RTS 내림 — 리셋을 피해본다)" % port[0])
time.sleep(0.3)
...
print("\r" + " " * 40 + "\r", end="")
if box["booted"]:
    print("  ※ 리셋이 걸렸다 (%s). 바퀴는 안 움직였으니 중앙 위치는 그대로다."
          % box["booted"])
